"""Phase 3 contract for the retire-at-62 article."""
from __future__ import annotations
import re
from pathlib import Path

ROOT = Path(__file__).resolve().parents[2]
ARTICLE = ROOT / "blog" / "how-much-to-retire-at-62" / "index.html"
...
def test_title_le_60_chars_with_primary_keyword():
    ...
    assert "62" in title and ("retire" in title.lower())
...
def test_h1_present_with_keyword():
    ...
    assert m
    h1 = m.group(1).lower()
    assert "62" in h1 and "retire" in h1
...
def test_cross_links_to_age_series():
    html = _html()
    for sibling in ("how-much-to-retire-at-55", "how-much-to-retire-at-60", "how-much-to-retire-at-65"):
